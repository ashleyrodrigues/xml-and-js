...
const renderTable = (data) => {
  ...
  if (window.location.search) {
    ...
    document.getElementById(`search`).value = nameTerm;
  ...
  const rows = data.map(
    ({ id, first_name, last_name, email, gender, ip_address }) => {
      table.innerHTML += `<tr id="table-row-${id}"><td>${id}</td><td>${first_name}</td><td>${last_name}</td><td>${email}</td><td>${gender}</td><td>${ip_address}</td></tr>`
    });
};
